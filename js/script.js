...
// runs if the language is changed
...
// check if left or right key is down
...
// calculate the new coordinates for Simulate
...
// calculate the coords for Simulate 
function calc_coord()
{			
	degrees = make_degrees();
...
// check the inpath for moje space or new lines
...
// get new step from the inpath textarea
function get_new_step()
{
	if (steps[last_step + 1] * 1 > 0) { // check if there are more steps
		...
		last_step += 2;
			   ...
			   steps[last_step + 1] == undefined || last_step == 1 ) && 
...
// check if the inpath step is coord
...
// display on canvas the robo and wheels
...
// display on canvas lines of the wheels
...
// replace robo_path textarea with out_path textarea value
...
// display on canvas and textarea crash message
function crash_mess()
{
	ctx.fillStyle = "white";	
	if (lang == 'e') {
...
// runs if the Stop button is down
function stopgen()
{
	document.getElementById('out_path').value 	 += '\n' + "Stoped!";			
	document.getElementById('out_path-bg').value += '\n' + "Спряно!";			
